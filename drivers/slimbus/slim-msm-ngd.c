				...
				void *_cmd)
{
	...
	case QMI_SERVER_ARRIVE:
		sr = schedule_work(&qmi->ssr_up);
		SLIM_INFO(dev, "schedule ret = %d\n", sr);
		break;
	default:
		break;
	}
	...
	if (enable) {
		...
		if (!ret) {
			...
			if (!pm_runtime_enabled(dev->dev) ||
				...
				SLIM_INFO(dev, "calling runtime resume\n");
				ret = pm_runtime_resume(dev->dev);
				SLIM_INFO(dev, "resume ret = %d\n", ret);
				ret = 0;
			}
			pm_runtime_mark_last_busy(dev->dev);
...
static int ngd_notify_slaves(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		SLIM_INFO(dev, "%s: In notification thread\n", __func__);
