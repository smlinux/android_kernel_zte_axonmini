...
static int qpnp_pon_store_and_clear_warm_reset(struct qpnp_pon *pon)
{
	...
	if (rc) {
       ...
       dev_err(&pon->spmi->dev, "qpnp pon reason1=0x%x, reason2=0x%x\n", 
                                                    pon->warm_reset_reason1, pon->warm_reset_reason2);
...
qpnp_pon_input_dispatch(struct qpnp_pon *pon, u32 pon_type)
{
	...
	if(cfg->pon_type == PON_KPDPWR)
		pr_info("PMIC input: code=%d, status=0x%x\n",
...
static int qpnp_pon_probe(struct spmi_device *spmi)
{
	...
	dev_info(&pon->spmi->dev,
				"PMIC@SID%d: pon_sts = 0x%x, poff_sts = 0x%x\n",
				pon->spmi->sid, pon_sts, poff_sts);
