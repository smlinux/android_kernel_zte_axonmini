	...
	if (on) {
		if (vib->mode != QPNP_VIB_MANUAL)
			pwm_enable(vib->pwm_info.pwm_dev);
                     ...
                     val |= ( ((on & 0xFF0000)>>16) & QPNP_VIB_VTG_SET_MASK);
                     rc = qpnp_vib_write_u8(vib, &val, QPNP_VIB_VTG_CTL(vib->base));
                     if (rc < 0)
                            printk(KERN_ERR"%s: set vib voltage failed!\n", __func__);
                     vib->reg_vtg_ctl = val;
                     //printk(KERN_ERR"%s: set vib voltage 0x%x\n", __func__, val);
...
static void qpnp_vib_enable(struct timed_output_dev *dev, int value)
{
	...
	hrtimer_cancel(&vib->vib_timer);
       ...
       pr_info("qpnp_vib_enable value = %d\n",value);
	...
	if (value == 0)
		vib->state = 0;
              ...
              if (vtg_value == 0 )
                     vtg_value = 0x1B0000;/* 1B: default value 2.7v */
		timeout_value = ( timeout_value > vib->timeout ?
				 vib->timeout : timeout_value);
		vib->state = 1 | vtg_value;
