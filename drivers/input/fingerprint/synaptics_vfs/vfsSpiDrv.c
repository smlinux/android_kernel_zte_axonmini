...
static int vfsspi_compat_rw_spi_message(struct vfsspi_device_data *vfsspi_device,
				 unsigned long arg)
{
			...
			unsigned long arg)
{
	...
	case VFSSPI_IOCTL_RW_SPI_MESSAGE:
		...
		ret_val = vfsspi_compat_rw_spi_message(vfsspi_device, arg);
		break;
	...
	case VFSSPI_IOCTL_SELECT_DRDY_NTF_TYPE:
        {
            ...
            if (copy_from_user(&drdyTypes, (void *)arg,
                ...
                if (copy_to_user((void *)arg, &(drdyTypes),
                    sizeof(vfsspi_iocSelectDrdyNtfType_t)) == 0) {
                        ret_val = 0;
                } else {
...
#ifdef CONFIG_COMPAT
static long
vfsspi_compat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	return vfsspi_ioctl(filp, cmd, (unsigned long)compat_ptr(arg));
}
#else
#define vfsspi_compat_ioctl NULL
#endif /* CONFIG_COMPAT */
...
static int vfsspi_probe(struct spi_device *spi)
{
	...
	if(spi->dev.of_node)
	{
		...
		vfsspi_device->sovcc_en_pin = of_get_named_gpio(spi->dev.of_node, "sovcc-en-gpio", 0);
		pr_info("%s : drdy-gpio = %d, sleep-gpio =%d\n", __func__, vfsspi_device->drdy_pin, vfsspi_device->sleep_pin);
		pr_info("%s : sovcc-en-gpio = %d\n", __func__, vfsspi_device->sovcc_en_pin);
	}
	else
	...
	if (gpio_request(vfsspi_device->sovcc_en_pin, "vfsspi_sovcc")) {
		pr_err("%s : gpio vfsspi_sovcc request failed\n", __func__);
		status = -EBUSY;
		goto vfsspi_probe_sovcc_failed;
	}

	status = gpio_direction_output(vfsspi_device->sovcc_en_pin, 1);
	if (status < 0) {
		pr_err("gpio_direction_output SOVCC failed\n");
		status = -EBUSY;
		goto vfsspi_probe_gpio_init_failed;
	}

	vcc = regulator_get(&(spi->dev), "vcc");
	if (IS_ERR(vcc)) {
		pr_err("%s: Failed to get regulator vcc\n", __func__);
		status = -EBUSY;
		goto vfsspi_probe_gpio_init_failed;
	}
	...
	if (status) {
		pr_err("%s: Failed to enable regulator vcc\n", __func__);
		regulator_put(vcc);
		status = -EBUSY;
		goto vfsspi_probe_gpio_init_failed;
	}
...
#endif
	gpio_free(vfsspi_device->sovcc_en_pin);
