...
static irqreturn_t ak49xx_irq_thread(int irq, void *data)
{
	...
	if (unlikely(ak49xx_lock_sleep(ak49xx_res) == false)) {
		...
		pr_err("[YCH]Failed to hold suspend\n");
		return IRQ_NONE;
	}
	...
	if (ret < 0) {
		...
		printk("[YCH] %s Failed to read interrupt status: %d\n",__func__,ret);
		return IRQ_NONE;
	} else {
		status[0] = ret;
	}
...
int ak49xx_irq_init(struct ak49xx_core_resource *ak49xx_res)
{
	...
	for (i = 0; i < AK4961_NUM_IRQS; i++) {
		...
		pr_debug("%s: irq %d -> %d\n", __func__, i, virq);
		if (virq == NO_IRQ) {
			pr_err("[YCH]%s, No interrupt specifier for irq %d\n",
				   __func__, i);
			return NO_IRQ;
		}
		...
		if (ret) {
			pr_err("[YCH]%s: Failed to configure irq %d (%d)\n",
				   __func__, i, ret);
			return ret;
		}
	...
	if (ret != 0)
		...
		dev_err(ak49xx_res->dev, "Failed to request IRQ %d: %d\n",
			ak49xx_res->irq, ret);
		pr_err("[YCH]Failed to request IRQ %d: %d\n",
			ak49xx_res->irq, ret);
		}
	else {
		...
		if (ret)
			...
			dev_err(ak49xx_res->dev, "Failed to set wake interrupt on"
				" IRQ %d: %d\n", ak49xx_res->irq, ret);
			pr_err("[YCH]Failed to set wake interrupt on"
				" IRQ %d: %d\n", ak49xx_res->irq, ret);
			}
		if (ret)
	...
	if (ret) {
		pr_err("[YCH]%s: Failed to init ak49xx irq\n", __func__);
