...
static void ak49xx_bring_up(struct ak49xx *ak49xx)
{
	...
	ret = ak49xx_interface_reg_write(ak49xx, AK496X_SLIM_PGD_PORT8_ARRAY, 0x02);
	ret = ak49xx_interface_reg_write(ak49xx, AK496X_SLIM_PGD_PORT9_ARRAY, 0x04);

	if(ret != 0){
		dev_err(ak49xx->dev, "%s: codec bring up ret = %d\n", __func__, ret);
	}else{
		dev_info(ak49xx->dev, "%s: codec bring up ret = %d\n", __func__, ret);
	}
}
...
static int ak49xx_reset(struct ak49xx *ak49xx)
{
	...
	printk(KERN_ERR "[lvrg] ak49xx_reset Entry");
#if 1
#if 0
	if (ak49xx->en_2p5_gpio) {    //2p5
		ret = gpio_request(ak49xx->en_2p5_gpio, "AKM_2P5_ENABLE");
...
static int ak49xx_device_init(struct ak49xx *ak49xx)
{
	...
	//	printk("[lhs] %s spi read chip ID =0x%x !\n",__func__,ret);	
...
static struct ak49xx_pdata *ak49xx_populate_dt_pdata(struct device *dev)
{
    ...
    printk(KERN_ERR "[lvrg] ak49xx_populate_dt_pdata Entry");
	...
	if (ret)
		goto err;
#if 0
	pdata->en_2p5_gpio = of_get_named_gpio(dev->of_node,
				"akm,cdc-2p5-en-gpio", 0);
	printk("%s pdata->en_2p5_gpio : %d\n",__func__,pdata->en_2p5_gpio);	
	if (pdata->en_2p5_gpio < 0) {
...
static int ak49xx_slim_probe(struct slim_device *slim)
{
    ...
    printk("ak49xx_slim_probe Entry\n");
	...
	//ak49xx->en_2p5_gpio = pdata->en_2p5_gpio;
	...
	pr_info("%s: lvrg get slimbus %s logical address: %d\n",
...
static int es704_power_on(struct device *dev)
{
	...
	if (IS_ERR(ak4961_mclk)) {
		printk(KERN_ERR "%s: Error getting ak4961_mclk\n", __func__);
		ak4961_mclk = NULL;
	}
	else{
		printk("%s: start prepare ak4961_mclk\n", __func__);
        ret = clk_prepare_enable(ak4961_mclk);
		if(ret){
		    printk(KERN_ERR "%s: prepare ak4961_mclk failed ret:%d\n", __func__, ret);
	    }
	}	
...
*/
		...
		ak49xx->write_dev = ak49xx_spi_write;
#endif
/*modify for ak49xx irq disable after insert a sim card by yangchao 20150717 begin*/
	dev_info(ak49xx->dev, "[lhs]%s:  ak49xx_res->irq=%d \n", __func__,ak49xx_res->irq);
#if 1
       if(!ak49xx_res->irq)
       {
...
/*modify for ak49xx irq disable after insert a sim card by yangchao 20150717 end*/
	return ret;
}
...
/*modify for ak49xx irq disable after insert a sim card by yangchao 20150717 end*/
	dev_err(ak49xx->dev, "YCH: %s: device down\n", __func__);
	printk("YCH %s out\n", __func__);
	return 0;
}
