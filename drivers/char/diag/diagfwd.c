...
void diag_flush_wq_work_fn(struct work_struct *work)
{
	struct diag_smd_info *smd_info = &driver->smd_data[driver->curr_peripheral];
				  ...
				  int ctxt)
{
	...
	case SMD_DATA_TYPE:
		if (peripheral >= 0 && peripheral < NUM_SMD_DATA_CHANNELS) {
			 ...
			 */
			if (driver->logging_mode == MEMORY_DEVICE_MODE) {
				driver->curr_peripheral = peripheral;
				schedule_work(&driver->diag_flush_wq_work);
			}
...
int diagfwd_init(void)
{
	...
	INIT_WORK(&(driver->diag_flush_wq_work), diag_flush_wq_work_fn);
