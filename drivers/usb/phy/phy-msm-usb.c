...
int input_current_mA_globle=500;
extern int g_boot_charger;
extern int bq24296_set_input_current(int ma);
...
static void msm_otg_notify_charger(struct msm_otg *motg, unsigned mA)
{
	...
	dev_info(motg->phy.dev, "Avail curr from USB = %u\n", mA);
       ...
       if (mA >= 500)
       {
              input_current_mA_globle = mA;
              queue_work(system_nrt_wq, &motg->set_input_current_work);
       }
...
static void set_input_current_work(struct work_struct *w)
{
...
extern int bq24296_enable_otg_globe(bool enable);
...
static int msm_otg_probe(struct platform_device *pdev)
{
	...
	if (!motg->phy.otg) {
       ...
       if ( g_boot_charger )
       {
              pr_err("poweroff charger, enable aca feature\n");
              debug_aca_enabled = true;
       }
	...
	pdata->vbus_power = bq24296_enable_otg_globe;
	...
	INIT_WORK(&motg->sm_work, msm_otg_sm_work);
       INIT_WORK(&motg->set_input_current_work, set_input_current_work);
