					...
					struct usb_composite_dev *cdev)
{
	...
	if (dev->pdata && dev->pdata->default_luns != 0) {
		config->fsg.nluns = 1;
...
static int android_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		ret = of_property_read_u32(pdev->dev.of_node, "qcom,android-usb-default-luns",
				   &(pdata->default_luns));
		if (ret) {
			pr_debug("no default luns set, set it ot 1.\n");
			pdata->default_luns = 1;
		}

		if(pdata->cdrom == 0 && pdata->default_luns  == 0){
			pr_err("Function no lun set.\n");
			return -EINVAL;
		}
