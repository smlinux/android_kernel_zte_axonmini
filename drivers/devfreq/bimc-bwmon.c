...
static int start_bw_hwmon(struct bw_hwmon *hw, unsigned long mbps)
{
	...
	u32 limit;
	int ret;
	trace_printk(" %s: call request_threaded_irq bimc-bwmon IRQ \n", __func__);
...
static void stop_bw_hwmon(struct bw_hwmon *hw)
{
	...
	trace_printk(" %s: going to free bimc-bwmon IRQ \n", __func__);
	mon_irq_disable(m);
	free_irq(m->irq, m);
	mon_disable(m);
	mon_clear(m);
...
static int resume_bw_hwmon(struct bw_hwmon *hw)
{
	struct bwmon *m = to_bwmon(hw);
	int ret;
	trace_printk(" %s: going to call request_threaded_irq bimc-bwmon IRQ \n", __func__);
	mon_clear(m);
	ret = request_threaded_irq(m->irq, NULL, bwmon_intr_handler,
				  IRQF_ONESHOT | IRQF_SHARED,
				  dev_name(m->dev), m);
