			...
			struct device_attribute *devattr, char *buf)
{
	...
	return snprintf(buf, QPNP_ADC_HWMON_NAME_LENGTH,
		"Result:%lld Raw:%x Voltage: %lld\n", result.physical, result.adc_code, result.measurement);
}
...
static ssize_t zte_read_hver_material(char *str)
{
    struct qpnp_vadc_result result;
    int rc = -1;
    int len=0;
    printk("zte_read_hver_material \n");
    rc = qpnp_vadc_read(vadc_tmp, P_MUX4_1_1, &result);
    if (rc) {
        pr_err("VADC read error with %d\n", rc);
        return 0;
    }
    printk(KERN_ERR"%s: result=%lld\n", __func__, result.physical);

    /* (3.9k/(100k+3.9k))*1.8v = 0.067564v, +-0.02v */
    if(result.physical > 1698000 && result.physical < 1728000){
        snprintf(str ,5,"bomB\n");
        bom_id = 'B';
        len = 5;
    }
    else if(result.physical > 1592000 && result.physical < 1622000){
        snprintf(str ,5,"bomC\n");
        bom_id = 'C';
        len = 5;
    }
    else if(result.physical > 1485000 && result.physical < 1515000){
        snprintf(str ,5,"bomD\n");
        bom_id = 'D';
        len = 5;
    }
    else if(result.physical > 1402000 && result.physical < 1432000){
        snprintf(str ,5,"bomE\n");
        bom_id = 'E';
        len = 5;
    }
    else if(result.physical > 1336000 && result.physical < 1366000){
        snprintf(str ,5,"bomF\n");
        bom_id = 'F';
        len = 5;
    }
	else if(result.physical > 1107000 && result.physical < 1137000){
        snprintf(str ,5,"bomG\n");
        bom_id = 'G';
        len = 5;
    }
	else if(result.physical > 1056000 && result.physical < 1086000){
        snprintf(str ,5,"bomH\n");
        bom_id = 'H';
        len = 5;
    }
	else if(result.physical > 961000 && result.physical < 991000){
        snprintf(str ,5,"bomI\n");
        bom_id = 'I';
        len = 5;
    }
	else if(result.physical > 885000 && result.physical < 915000){
        snprintf(str ,5,"bomJ\n");
        bom_id = 'J';
        len = 5;
    }
	else if(result.physical > 778000 && result.physical < 808000){
        snprintf(str ,5,"bomK\n");
        bom_id = 'K';
        len = 5;
    }
    else if(result.physical > 642000 && result.physical < 672000){
        snprintf(str ,5,"bomL\n");
        bom_id = 'L';
        len = 5;
    }
    else if(result.physical > 585000 && result.physical < 615000){
        snprintf(str ,5,"bomM\n");
        bom_id = 'M';
        len = 5;
    }
    else if(result.physical > 376000 && result.physical < 406000){
        snprintf(str ,5,"bomN\n");
        bom_id = 'N';
        len = 5;
    }
    else if(result.physical > 298000 && result.physical < 328000){
        snprintf(str ,5,"bomO\n");
        bom_id = 'O';
        len = 5;
    }
...
static int material_proc_show(struct seq_file *m, void *v)
{
       zte_read_hver_material(bom_id_str);
	seq_printf(m, "%s\n", bom_id_str);
	return 0;
}

static int material_proc_open(struct inode *inode, struct file *file)
{
        return single_open(file, material_proc_show, NULL);
}

static const struct file_operations material_proc_fops = {
	.open		= material_proc_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static int zte_creat_hver_material_proc_file(void)
{
	struct proc_dir_entry *pe;

	pe = proc_create("driver/hardwareVersionMaterial", 0444, NULL, &material_proc_fops);
	if (!pe)
		return -ENOMEM;
	return 0;
}

char zte_read_bom_id(void)
{
    zte_read_hver_material(bom_id_str);
    printk(KERN_ERR"%s: %s, %c\n", __func__, bom_id_str, bom_id);
    return bom_id;
}
EXPORT_SYMBOL(zte_read_bom_id);
...
static int qpnp_vadc_probe(struct spmi_device *spmi)
{
	...
	list_add(&vadc->list, &qpnp_vadc_device_list);
       ...
       if (0)
       {
              snprintf(bom_id_str,5,"Null\n");
        	zte_creat_hver_material_proc_file();
        	bom_id = zte_read_bom_id();
       }
	return 0;
...
__define_initcall(qpnp_vadc_init, 5s);
