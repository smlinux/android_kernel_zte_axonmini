...
static bool msm_pm_pc_hotplug(void)
{
	...
	trace_printk(" %s: called= %pS  cpu=%d gic_is_any_irq_pending =%d line = %d\n", __func__, __builtin_return_address(0), cpu, gic_is_any_irq_pending(), __LINE__);
	if(gic_is_any_irq_pending()){
