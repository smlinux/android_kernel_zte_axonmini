...
int msm_get_dload_mode()
{
	return download_mode;
}
EXPORT_SYMBOL(msm_get_dload_mode);
...
static void msm_restart_prepare(const char *cmd)
{
	...
	if (cmd != NULL) {
		if (!strncmp(cmd, "bootloader", 10)) {
			...
			if (!ret){
				pr_info("reboot oem-%02lx\n", code);
                            if ( code == 0x8 )
                                   qpnp_pon_set_restart_reason(PON_RESTART_REASON_USER_POWER_KEY);
				__raw_writel(0x6f656d00 | (code & 0xff),
					     restart_reason);
			}
		} else if (!strncmp(cmd, "edl", 3)) {
			enable_emergency_dload_mode();
		} else if (!strncmp(cmd, "pwrk", 4)){
		       qpnp_pon_set_restart_reason(PON_RESTART_REASON_POWEROFF_CHARGING);
			__raw_writel(0x77665505, restart_reason); 
              }else if (!strncmp(cmd, "ftmmode", 7)) {
			qpnp_pon_set_restart_reason(
				PON_RESTART_REASON_FTM);
			__raw_writel(0x6f656d0f, restart_reason);
		} else {
