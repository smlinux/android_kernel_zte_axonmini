...
static int qpnp_charger_read_dt_props(struct qpnp_lbc_chip *chip)
{
	...
	if (chip->cfg_use_fake_battery)
       ...
       chip->cfg_use_external_charger = false;
...
extern int bq24296_enable_charging_globle(int batt_temp);
#define TEMP_WORK_CYCLE		msecs_to_jiffies(20000)
#define DELAY_START_TEMP_WORK msecs_to_jiffies(5000)
static int flag_batt_temp_irq_trigered = 0;

static void qpnp_batt_temp_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct qpnp_lbc_chip *chip = container_of(dwork,
				struct qpnp_lbc_chip, batt_temp_work);
       int batt_temp;
       int usb_present = 0;
       union power_supply_propval ret = {0,};
       static int batt_temp_over_count = 0;

       batt_temp = get_prop_batt_temp(chip);
       
       //pr_err("%s:  batt_temp=%d\n", __func__, batt_temp);
    
       if (chip->ac_psy ){
             chip->ac_psy->get_property(chip->ac_psy, POWER_SUPPLY_PROP_PRESENT, &ret);
             usb_present = ret.intval;
       }

       bq24296_enable_charging_globle(batt_temp);
       
       if (  ( (batt_temp > 50) && (batt_temp < 400) )  || usb_present==0)
       {            
            if ( flag_batt_temp_irq_trigered == 1 )
            {
                 flag_batt_temp_irq_trigered = 0;
                 pr_err("%s:  usb_present =%d. relax temp_work lock.\n", __func__, usb_present);
                 pm_relax(chip->dev);
            }
       }
       ...
       if (batt_temp >= 685)
       {
            batt_temp_over_count++;
            pr_err("batt_temp is over 68C, batt_temp_over_count =%d\n", batt_temp_over_count);
            if(batt_temp_over_count >=3)
            {
                pr_err("batt_temp is over 68C 3 times, power off\n");
                emergency_sync();
                kernel_power_off();
            }
       }
       ...
       {
                batt_temp_over_count = 0;
       }

       schedule_delayed_work(&chip->batt_temp_work, TEMP_WORK_CYCLE);
       
	return;

}

#define TEMP_WORK_DELAY		msecs_to_jiffies(50)
static irqreturn_t qpnp_lbc_batt_temp_irq_handler(int irq, void *_chip)
{
	struct qpnp_lbc_chip *chip = _chip;

       pm_stay_awake(chip->dev);
       flag_batt_temp_irq_trigered = 1;
       pr_err("##  lock! pm_stay_awake\n");
       schedule_delayed_work(&chip->batt_temp_work, TEMP_WORK_DELAY);
...
extern struct bq24296_chip *the_bq24296_chip;
...
static irqreturn_t qpnp_lbc_fastchg_irq_handler(int irq, void *_chip)
{
	...
	//struct qpnp_lbc_chip *chip = _chip;    
       ...
       pr_err("%s ######### FAST_CHG IRQ triggered, fastchg_on\n", __func__);       
...
static int qpnp_lbc_probe(struct spmi_device *spmi)
{
	...
	pr_info("Probe chg_dis=%d bpd=%d usb=%d batt_pres=%d batt_volt=%d soc=%d\n",
       ...
       schedule_delayed_work(&chip->batt_temp_work, DELAY_START_TEMP_WORK);
