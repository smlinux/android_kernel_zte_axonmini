...
#define NORMAL_BATT_MAX_VOLTAGE_UV 4350000
...
struct qpnp_bms_chip {
	...
	int				reported_soc_delta;
       bool                      start_soc_catch_up;
};

extern bool globle_charging_done;
static struct qpnp_bms_chip *the_vm_bms_chip;
static enum batt_id_type get_battery_oem(struct qpnp_bms_chip *chip);
...
static int __init recovery_mode_setup(char *str)
{
    ...
    printk(KERN_NOTICE "@@ %s recoverymod=%d\n", __func__, recovery_mode);
    return 1;
}
__setup("zte.recoverymod=", recovery_mode_setup);
...
int qpnp_set_usb_suspend( bool enable)
{
	int rc;
	u8 reg;
       u16 addr = 0x1347;
	...
	rc = qpnp_write_wrapper(the_vm_bms_chip, &reg, addr, 1);
	if (rc)
		pr_err("write failed addr = %03X, val = %02x, rc = %d\n", addr, reg, rc);
...
static int get_battery_voltage(struct qpnp_bms_chip *chip, int *result_uv)
{
	...
	*result_uv = (int)adc_result.physical;
       ...
       if ((int)adc_result.physical < 3400000)
       {
           if ( ftm_mode == 1 || recovery_mode == 1 )
           {
                  pr_err("in FTM or recovery mode, vbat=%d too low, power off!\n", (int)adc_result.physical);
                  emergency_sync();
                  kernel_power_off();
           }
       }
		...
		if (chip->charger_reinserted) {
			 ...
			 */
                     if (globle_charging_done)
                     {
                            ...
                            chip->reported_soc = 100;
		              chip->reported_soc_delta = 100 - chip->last_soc;
		              pr_err("Re-inserted and full, Keep at reported_soc 100, reported_soc_delta=%d, last_soc=%d\n",
						chip->reported_soc_delta,
						chip->last_soc);
                     }
			else if (chip->reported_soc == chip->last_soc) {
...
#define SOC_CATCHUP_SEC_MAX		3000
#define SOC_CATCHUP_SEC_PER_PERCENT	60
#define MAX_CATCHUP_SOC	(SOC_CATCHUP_SEC_MAX / SOC_CATCHUP_SEC_PER_PERCENT)
#define SOC_CHANGE_PER_SEC_DISCHG		30
#define SOC_CHANGE_PER_SEC_CHG		5

int globle_vm_bms_soc = 50;
static int report_vm_bms_soc(struct qpnp_bms_chip *chip)
{
	...
	bool charging;
       static int soc_start_catch_up = 0;
       static bool poweron_first_enter = true;

	soc = chip->calculated_soc;

	last_change_sec = chip->last_soc_change_sec;
       pr_err("## enter: last_change_sec=%ld\n", last_change_sec);
	calculate_delta_time(&last_change_sec, &time_since_last_change_sec);

       if ( poweron_first_enter )
       {
               chip->last_soc_change_sec = last_change_sec;
               time_since_last_change_sec = 0;
               poweron_first_enter = false;
       }
       pr_err("## last_change_sec=%ld, time_since_last_change_sec=%d\n", 
                                       last_change_sec, time_since_last_change_sec);
	...
	if (charging && chip->last_soc != -EINVAL) {
		if (chip->charge_start_tm_sec == 0 ||( globle_charging_done && !chip->start_soc_catch_up)) {

                     if ( globle_charging_done )
                            chip->start_soc_catch_up = true;
			...
			if (abs(soc - chip->last_soc) < MAX_CATCHUP_SOC)
				...
				chip->catch_up_time_sec = SOC_CATCHUP_SEC_MAX;
                     ...
                     soc_start_catch_up = chip->last_soc;

			if (chip->catch_up_time_sec < 0)
				chip->catch_up_time_sec = 0;
			chip->charge_start_tm_sec = last_change_sec;
                     pr_err("## charge_start_tm_sec=%ld. SOC=%d, last_soc=%d, catch_up_time_sec=%d\n", 
                                                    last_change_sec, soc, chip->last_soc, chip->catch_up_time_sec);
		}

		charge_time_sec = min(SOC_CATCHUP_SEC_MAX, (int)last_change_sec
				- chip->charge_start_tm_sec);
              pr_err("## charge_time_sec=%d\n", charge_time_sec);
		/* end catchup if calculated soc and last soc are same */
		if (chip->last_soc == soc)
              {      
			chip->catch_up_time_sec = 0;
                     chip->start_soc_catch_up = false;
                     pr_err("## chip->last_soc == soc\n");
              }
	}
	...
	if (chip->last_soc != -EINVAL) {
		...
		if (chip->last_soc < soc && !charging)
			soc = chip->last_soc;
		else if (chip->last_soc < soc)
			soc = scale_soc_while_chg(chip, charge_time_sec,
					chip->catch_up_time_sec,
					soc, soc_start_catch_up);
		...
		if (batt_temp <= chip->dt.cfg_low_temp_threshold)
			soc_change = min((int)abs(chip->last_soc - soc),
		...
		else
              {
			    ...
			    soc_change = min((int)abs(chip->last_soc - soc),
				time_since_last_change_sec
					/ SOC_CHANGE_PER_SEC_CHG);
                     else
                         soc_change = min((int)abs(chip->last_soc - soc),
				time_since_last_change_sec
					/ SOC_CHANGE_PER_SEC_DISCHG);
              }
	...
	if (chip->last_soc != soc && !chip->last_soc_unbound)
       {   
              pr_err("## last_soc_change_sec=%d, last_change_sec=%ld\n", 
                                       chip->last_soc_change_sec, last_change_sec);
		chip->last_soc_change_sec = last_change_sec;
       }
	...
	pr_err("## last_soc=%d calculated_soc=%d soc=%d time_since_last_change=%d, soc_change=%d\n",
			chip->last_soc, chip->calculated_soc,
			soc, time_since_last_change_sec, soc_change);


	if (chip->reported_soc_in_use)
       {
              globle_vm_bms_soc = prepare_reported_soc(chip);

              backup_ocv_soc(chip, chip->last_ocv_uv, globle_vm_bms_soc);
              return globle_vm_bms_soc;
       }
    
       globle_vm_bms_soc = chip->last_soc;
	...
	pr_err("Reported SOC=%d, batt_temp=%d\n", chip->last_soc, batt_temp);
...
static void calculate_reported_soc(struct qpnp_bms_chip *chip)
{
	...
	if (chip->reported_soc > chip->last_soc) {
		...
		pr_err("%s New reported_soc=%d, last_soc is=%d\n",
					__func__, chip->reported_soc, chip->last_soc);
	} else {
		chip->reported_soc_in_use = false;
		pr_err("%s reported_soc equals last_soc, stop process\n", __func__);
	}
	pr_debug("bms power_supply_changed\n");
	power_supply_changed(&chip->bms_psy);
}

struct sustain_time_entry {
	int  current_mA;
	int  seconds;
};

static struct sustain_time_entry sustain_time_table_high_soc[] = {
       {900, 120},
       {800, 180},
       {700, 240},
	{600, 300},
...
/*  seconds = (2800mAh*1%*3600/current_mA)   */
static struct sustain_time_entry sustain_time_table_other_soc[] = {
       {2400, 42},
       {2100, 48},
       {1800, 56},
       {1500, 67},
...
#define CALCULATE_DELAY_MS_FOR_SUSTAIN 10000
...
static void monitor_soc_work(struct work_struct *work)
{
	...
	int rc, vbat_uv = 0, new_soc = 0, batt_temp;
       int i;
	static int sustain_time =0, i_samples_temporary=0;
       static int  i_sum=0, iavg_temporary=0;
	...
	if (!is_battery_present(chip)) {
		/* if battery is not preset report 50% SOC */
		pr_err("battery gone, reporting 50\n");
		chip->last_soc_invalid = true;
		...
		if (chip->dt.cfg_use_voltage_soc) {
			...
			new_soc = lookup_soc_ocv(chip, chip->last_ocv_uv, batt_temp);
                     ...
                     if ( globle_charging_done )
                     {
                            ...
                            pr_err("#1 Charge terminated, set chip->calculated_soc = 100\n");
                            if ( 100 == report_vm_bms_soc(chip))
                                globle_charging_done = false;
                     }
			else if (chip->calculated_soc != new_soc) {
                            pr_err("#2 SOC changed! new_soc=%d prev_soc=%d\n",
...
-                                report_vm_bms_soc(chip);
				pr_err("#2 update bms_psy\n");
				power_supply_changed(&chip->bms_psy);
			} else if (chip->last_soc != chip->calculated_soc) {
				pr_err("#3 update bms_psy\n");
				power_supply_changed(&chip->bms_psy);
			} else {
			       pr_err("#4 enter report_vm_bms_soc\n");
				report_vm_bms_soc(chip);
			}
		}
	...
	if (chip->reported_soc_in_use && chip->charger_removed_since_full
		...
		/* record the elapsed time after last reported_soc change */
              if( chip->reported_soc_change_sec == 0)
              {
                   chip->reported_soc_change_sec = 1;
                   i_sum = chip->current_now/1000;
                   i_samples_temporary =1;
                   iavg_temporary = i_sum;
              }
              else
              {
		    chip->reported_soc_change_sec += chip->delta_time_s;
                  i_samples_temporary +=1;
                  i_sum += (chip->current_now/1000);
                  iavg_temporary = i_sum/i_samples_temporary;
              }

		/* above the catch up time, calculate new reported_soc */
              if ( chip->reported_soc > 97 )
              {
                     for (i = 0; i < ARRAY_SIZE(sustain_time_table_high_soc) -1; i++) {
        		     if (iavg_temporary  >= sustain_time_table_high_soc[i].current_mA)
        		     break;
        	       }
        	       sustain_time = sustain_time_table_high_soc[i].seconds;
              }
              else if ( chip->reported_soc > 90 )
              {
                    for (i = 0; i < ARRAY_SIZE(sustain_time_table_other_soc) -1; i++) {
        		     if (iavg_temporary >= sustain_time_table_other_soc[i].current_mA)
        		     break;
        	      }
        	      sustain_time = (sustain_time_table_other_soc[i].seconds);
              }
              else
              {
                    for (i = 0; i < ARRAY_SIZE(sustain_time_table_other_soc) -1; i++) {
        	      ...
        	      sustain_time = sustain_time_table_other_soc[i].seconds/2;
              }

              pr_err("reported_soc_change_sec=%d, sustain_time=%d, current=%d\n", 
                        chip->reported_soc_change_sec, sustain_time, iavg_temporary);
		if (chip->reported_soc_change_sec >= sustain_time) {
			calculate_reported_soc(chip);
			chip->reported_soc_change_sec = 0;
                     i_samples_temporary = 0;
                     iavg_temporary = 0;
                     i_sum = 0;
		}

              schedule_delayed_work(&chip->monitor_soc_work,
                       msecs_to_jiffies(CALCULATE_DELAY_MS_FOR_SUSTAIN) );
	}
...
static void voltage_soc_timeout_work(struct work_struct *work)
{
	...
	if (!chip->bms_dev_open) {
		pr_err("BMS device not opened, using voltage based SOC\n");
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CYCLE_COUNT:
       ...
       case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		...
		if (rc < 0)
			value = DEFAULT_BATT_VOL;
		val->intval = value;
              break;
       case POWER_SUPPLY_PROP_BATTERY_ID:              
		val->intval = get_battery_oem(chip);
              break;
	default:
		return -EINVAL;
	}
...
static void reported_soc_check_status(struct qpnp_bms_chip *chip)
{
	...
	if (present && chip->charger_removed_since_full) {
              if (chip->current_now > 0) {
                    chip->charger_reinserted = false;
                    pr_err("reported_soc: set reinserted false\n");
              }
              else {
       ...
       if (!present && chip->charger_removed_since_full && chip->charger_reinserted) {
		chip->charger_reinserted = false;
...
static int calculate_initial_soc(struct qpnp_bms_chip *chip)
{
	...
	pr_err("warm_reset=%d est_ocv=%d  shutdown_soc_invalid=%d shutdown_ocv=%d shutdown_soc=%d last_soc=%d calculated_soc=%d last_ocv_uv=%d\n",
...
static enum batt_id_type get_battery_oem(struct qpnp_bms_chip *chip)
{
	int64_t val;
       ...
       val = read_battery_id(chip);

       if (val > 1041000 && val < 1273000)
            return BATT_ID_LG;
       else if (val > 737000 && val < 901000)
            return BATT_ID_BAK;
       else if (val > 270000 && val < 330000)
            return BATT_ID_GY;
       else
...
static int qpnp_vm_bms_probe(struct spmi_device *spmi)
{
	...
	pr_err("probe success: soc=%d vbatt=%d ocv=%d warm_reset=%d\n",
