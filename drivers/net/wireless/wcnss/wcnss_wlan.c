...
#include <linux/proc_fs.h>  /** ZTE_MODIFY wangzhanmeng, add wifi chip identify in FTM PV   20150411*/
...
/** ZTE_MODIFY wangzhanmeng add wifi nv bin backup file 20150314 */
#define NVBIN_FILE_BACKUP "wlan/prima/WCNSS_qcom_wlan_nv_backup.bin"
...
/** ZTE_MODIFY wangzhanmeng, add wifi chip identify in FTM PV   20150411 */
#if 0
#include "../../../../../system/core/include/cutils/properties.h"
static const char SW_INTER_VER[] = "ro.build.sw_internal_version";
static int wcn_info_proc_show(struct seq_file *m, void *v)
{
	/* use below method or use method that if support 5G to distinguish WCN3680B between WCN3620*/
    char sw_inter_ver[PROPERTY_VALUE_MAX];
		
    property_get(SW_INTER_VER, sw_inter_ver, NULL);
	 if ( NULL != strchr(sw_inter_ver,"P839A01"))
	 {
	 	seq_printf(m,"QUALCOMM-WCN3680B-NA-NA-NA \n");
	       printk("%s:  WLAN chip id  is WCN3680B \n", __func__);
	 }
	 else if ( NULL != strchr(sw_inter_ver,"P874S30"))
	 {
		seq_printf(m,"QUALCOMM-WCN3620-NA-NA-NA \n");
		printk("%s:  WLAN chip id  is WCN3620 \n", __func__);
	 }
	 else
	 {
	 	seq_printf(m,"QUALCOMM-WCN3680B-NA-NA-NA \n");
...
static int wcn_info_proc_show(struct seq_file *m, void *v)
{
	...
	{
		seq_printf(m,"QUALCOMM-WCN3680B-NA-NA-NA \n");
		printk("%s,%d:  WLAN chip id  is WCN3680B \n", __func__,__LINE__);
	}
	else
	{
		seq_printf(m,"QUALCOMM-WCN3620-NA-NA-NA \n");
		printk("%s,%d:  WLAN chip id  is WCN3620 \n", __func__,__LINE__);
	}
	return 0;
}

#endif
static int wcn_info_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, wcn_info_proc_show, NULL);
}

static const struct file_operations proc_read_chip_info_file = {
	.open		= wcn_info_proc_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static int wcn_creat_chip_info_files(void)
{
	struct proc_dir_entry *refresh;

	refresh = proc_create("driver/wlan_id", 0644, NULL,&proc_read_chip_info_file);

	if (!refresh) {
		pr_err("%s: proc_create driver/wlan_id Failed !!!\n",__func__);
		return -ENOMEM;
	}

	pr_err("%s: proc_create driver/wlan_id Success !!!\n",__func__);


	refresh = proc_create("driver/bt_id", 0644, NULL,&proc_read_chip_info_file);

	if (!refresh) {
		pr_err("%s: proc_create driver/bt_id Failed !!!\n",__func__);
		return -ENOMEM;
	}

	pr_err("%s: proc_create driver/bt_id Success !!!\n",__func__);

	return 0;

}
/** ZTE_MODIFY wangzhanmeng, add wifi chip identify in FTM PV   end */
...
wcnss_ctrl_probe(struct platform_device *pdev)
{
	...
	wcn_creat_chip_info_files(); /** ZTE_MODIFY wangzhanmeng, add wifi chip identify in FTM PV   20150411 */
...
static void wcnss_nvbin_dnld(void)
{
	...
	/** ZTE_MODIFY wangzhanmeng modify for wifi nv bin backup file 20150314 */
	#if 0
	...
	#else
	ret = request_firmware(&nv, NVBIN_FILE, dev);
	pr_err("wcnss: %s: line=%d :request_firmware for %s\n",__func__, __LINE__,NVBIN_FILE);
	if (ret || !nv || !nv->data || !nv->size) {
		pr_err("wcnss: %s: request_firmware  failed for %s,NEXT will request_firmware BACKUP FILE\n",
			__func__, NVBIN_FILE);
		ret = request_firmware(&nv, NVBIN_FILE_BACKUP, dev);
		pr_err("wcnss: %s: line=%d :have request_firmware BACKUP FILE for %s\n",__func__, __LINE__,NVBIN_FILE_BACKUP);
		if (ret || !nv || !nv->data || !nv->size) {
			pr_err("wcnss: %s: request_firmware BACKUP FILE  failed for %s\n",
				__func__, NVBIN_FILE_BACKUP);
		goto out;
		}
	}
	#endif
	/** ZTE_MODIFY wangzhanmeng modify for wifi nv bin backup file   end */
