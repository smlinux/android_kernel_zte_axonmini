...
bool  wcnss_vreg_debug=false;//add by wangzhanmeng test 20150326
	...
	case WCNSS_PRONTO_HW:
		if (is_pronto_vt) {
			ret = wcnss_vregs_on(dev, iris_vregs_pronto_v2,
					ARRAY_SIZE(iris_vregs_pronto_v2));
					pr_err("wzm---%s,%d, use iris_vregs_pronto_v2 \n",__func__,__LINE__);//wangzhanmeng add for test 20160325

		} else {
			ret = wcnss_vregs_on(dev, iris_vregs_pronto,
					ARRAY_SIZE(iris_vregs_pronto));
					pr_err("wzm---%s,%d, use iris_vregs_pronto \n",__func__,__LINE__);//wangzhanmeng add for test 20160325
	...
	case WCNSS_PRONTO_HW:
		if (is_pronto_vt) {
			ret = wcnss_vregs_on(dev, pronto_vregs_pronto_v2,
					ARRAY_SIZE(pronto_vregs_pronto_v2));
				pr_err("wzm---%s,%d, use pronto_vregs_pronto_v2 \n",__func__,__LINE__);//wangzhanmeng add for test 20160325
					pr_err("wzm---%s,%d, use pronto_vregs_pronto_v3 \n",__func__,__LINE__);//wangzhanmeng add for test 20160325
		} else {
			ret = wcnss_vregs_on(dev, pronto_vregs,
					ARRAY_SIZE(pronto_vregs));
					pr_err("wzm---%s,%d, use pronto_vregs  \n",__func__,__LINE__);//wangzhanmeng add for test 20160325
		}
		break;
	default:
		pr_err("%s invalid hardware %d\n", __func__, hw_type);
		...
		enum wcnss_opcode on, int *iris_xo_set)
{
	...
	wcnss_vreg_debug = false;//add by wangzhanmeng for test 20150326
