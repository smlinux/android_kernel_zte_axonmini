...
extern void check_flag_zte_eeprom(void);
//////////
...
static int zte_eeprom_read(void *data)
{
    ...
    rc = msm_camera_power_up(e_ctrl->power_info, e_ctrl->eeprom_device_type,&e_ctrl->i2c_client);
    if (rc) 
    {
        pr_err("failed rc %d\n", rc);
        goto memdata_free;
   ...
   //////////////added for reduce eeprom time by chengjia 2015/8/6
   check_flag_zte_eeprom();
   ////////
    rc = msm_camera_power_down(e_ctrl->power_info, e_ctrl->eeprom_device_type,&e_ctrl->i2c_client);
    if (rc) 
    {
        pr_err("failed rc %d\n", rc);
        goto memdata_free;
    ...
    e_ctrl->is_supported = (e_ctrl->is_supported << 1) | 1;
    pr_err("zte_eeprom_read x\n");
    return rc;

    power_down:
    msm_camera_power_down(e_ctrl->power_info, e_ctrl->eeprom_device_type,
    &e_ctrl->i2c_client);
    memdata_free:
    kfree(e_ctrl->cal_data.mapdata);
    kfree(e_ctrl->cal_data.map);
