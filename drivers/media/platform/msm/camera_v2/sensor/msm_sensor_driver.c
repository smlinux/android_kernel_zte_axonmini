...
//chengjia added for read id start 2015/7/1
#include <linux/proc_fs.h>
#include <linux/seq_file.h>

static char g_back_camera_status [128]  = {0};
static char g_front_camera_status [128]  = {0};
static struct proc_dir_entry *g_back_camera_proc_file = NULL;
static struct proc_dir_entry *g_front_camera_proc_file = NULL;
//chengjia added for read id end
...
/////////////////////chengjia add for camera id info 2015/7/1
static int back_camera_id_read_proc(struct seq_file *m, void *v)
{		 	
    return seq_printf(m, "%s", g_back_camera_status);
}

static int back_camera_id_open_proc(struct inode *inode, struct file *file)
{
...
{   
    if(!strcmp("s5k3m2xm",s_ctrl->sensordata->sensor_name))
    {
    	sprintf(g_back_camera_status,"%s-%s-%s-%s-%s","NA","S5K3M2 13M","NA","NA","NA");
        g_back_camera_proc_file = proc_create("driver/camera_id_back",0644,NULL, &g_back_camera_proc_ops);
    	if(!g_back_camera_proc_file)
    	{
        	printk(KERN_INFO "back_camera_proc_file error!\r\n");
        	return -1;
    	}
    }
    return 0;
...
{		 	
    return seq_printf(m, "%s", g_front_camera_status);
}

static int front_camera_id_open_proc(struct inode *inode, struct file *file)
{
...
{
    .owner = THIS_MODULE,
    .open = front_camera_id_open_proc,
    .read = seq_read,
    .llseek = seq_lseek,
    .release = single_release,
};

static int32_t front_camera_proc_file(struct msm_sensor_ctrl_t *s_ctrl)
{   
    if(!strcmp("t4k35",s_ctrl->sensordata->sensor_name))
    {
    	sprintf(g_front_camera_status,"%s-%s-%s-%s-%s","NA","T4K35 8M","NA","NA","NA");
        g_front_camera_proc_file = proc_create("driver/camera_id_front",0644,NULL, &g_front_camera_proc_ops);
    if(!g_front_camera_proc_file)
    {
        printk(KERN_INFO "front_camera_proc_file error!\r\n");
        return -1;
    }
    	}
    return 0;
	...
	struct msm_sensor_info_t *probed_info, char *entity_name)
{
	...
	*  sensor control structure */
	...
	////////////chengjia add for proc id 2015/7/1
	printk("chengjiatest: add proc id\n");
