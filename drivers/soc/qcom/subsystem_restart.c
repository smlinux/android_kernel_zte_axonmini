...
static void subsystem_shutdown(struct subsys_device *dev, void *data)
{
	...
	if (dev->desc->shutdown(dev->desc, true) < 0){
		if(msm_get_dload_mode() == 1){
			panic( "subsys-restart: [%p]: Failed to shutdown %s!",
				current, name);
		}else{
			dev_err(dev->desc->dev, "subsys-restart: [%p]: Failed to shutdown %s!",
				current, name);
		}
	}
	dev->crash_count++;
...
static void subsystem_powerup(struct subsys_device *dev, void *data)
{
	...
	if (dev->desc->powerup(dev->desc) < 0) {
		...
		if(msm_get_dload_mode() == 1){
			...
			dev_err(dev->desc->dev, "[%p]: Powerup error: %s!", current, name);
			return;
		}
	}
	enable_all_irqs(dev);
	...
	if (ret) {
		...
		if(msm_get_dload_mode() == 1){
			...
			dev_err(dev->desc->dev, "[%p]: Timed out waiting for error ready: %s!",
				current, name);
			return;
		}
	}
...
int subsys_set_intentional_reset(struct subsys_device *dev)
{
	return dev->intentional_reset = 1;
}
EXPORT_SYMBOL(subsys_set_intentional_reset);
...
int subsystem_restart_dev(struct subsys_device *dev)
{
	...
	case RESET_SOC:
		if(dev->intentional_reset == 0){
			__pm_stay_awake(&dev->ssr_wlock);
			schedule_work(&dev->device_restart_work);
			return 0;
		}
	case RESET_SUBSYS_COUPLED:
		if(dev->intentional_reset  == 1){
			pr_err("Begin to modem intentional resetting.\n");
			dev->intentional_reset = 0;
		}
