...
static int mdss_dsi_panel_power_off(struct mdss_panel_data *pdata)
{
	...
	//zte-modify,yyp,optimise lcd driver(open lp11_init), begin
    mdss_dsi_panel_power_enable(pdata, 0);
    //zte-modify,yyp,optimise lcd driver(open lp11_init), end
end:
	return ret;
}
...
static int mdss_dsi_unblank(struct mdss_panel_data *pdata)
{
	...
	//	g_amoled_error_flag = 0;
		enable_irq(gpio_to_irq(ctrl_pdata->disp_err_gpio));
		pr_err("%s: enable_irq g_amoled_error_flag %d\n",	__func__, g_amoled_error_flag);
	}
...
static int mdss_dsi_blank(struct mdss_panel_data *pdata, int power_state)
{
	...
	if ((pdata->panel_info.type == MIPI_CMD_PANEL) &&
		...
		disable_irq(gpio_to_irq(	ctrl_pdata->disp_err_gpio));
	//	g_amoled_error_flag = 0;
		pr_err("%s: disable_irq g_amoled_error_flag %d\n",	__func__, g_amoled_error_flag);
...
static void acl_set_work_func(struct work_struct *work)
{
	mdss_dsi_panel_acl(&(g_ctrl_pdata->panel_data), g_ctrl_pdata->current_acl_level);

	return;
}
static void hbm_set_work_func(struct work_struct *work)
{
...
{	
	...
	if (ret == g_ctrl_pdata->current_acl_level)
		return 1;

	 g_ctrl_pdata->current_acl_level = ret;


	pr_err("%s : current_acl_level = %d\n", __func__, g_ctrl_pdata->current_acl_level);

	 if(first_time)
	 {
		queue_delayed_work(g_ctrl_pdata->acl_set_workqueue,
			&(g_ctrl_pdata->acl_set_work),
...
static int panel_acl_proc_init(void)
{
	...
	g_ctrl_pdata->current_acl_level = 1;
	res = proc_create("panel_acl_switch", S_IWUGO | S_IRUGO, NULL,
			  &panel_acl_proc_fops);
	if (!res)
	{
		printk(KERN_INFO "failed to create /proc/panel_acl_switch\n");
		return -ENOMEM;
	}
	printk(KERN_INFO "created /proc/panel_acl_switch\n");
	return 0;
}
...
//	static int first_time = 1;
	...
	if (ret == g_ctrl_pdata->current_hbm_level)
		return 1;
	...
	pr_err("%s : current_hbm_level = %d\n", __func__, g_ctrl_pdata->current_hbm_level);
	 ...
	 if( g_ctrl_pdata->current_hbm_level == 0)
	 {
        	mdss_dsi_panel_hbm(&(g_ctrl_pdata->panel_data), 0);
	 }
	 else if(g_ctrl_pdata->current_backlight_level == 255 && g_ctrl_pdata->current_hbm_level==1)
	 {
        	mdss_dsi_panel_hbm(&(g_ctrl_pdata->panel_data), 1);
	 }
...
void mdss_hbm_set_state(int enable)
{
	static int current_state = 0;
	static int count = 0;
	
    struct mdss_panel_info *pinfo = &(g_ctrl_pdata->panel_data.panel_info);    
	...
	if(enable == current_state)
	{
		if(enable == 1)
		{
			...
			if(count > 10000)
			{
				pr_err("%s : lip mdss_hbm_set_state count = %d\n", __func__, count);
				enable = 0;
				mdss_dsi_panel_hbm(&(g_ctrl_pdata->panel_data), enable);
	...
	mdss_dsi_panel_hbm(&(g_ctrl_pdata->panel_data), enable);
	current_state = enable;
	pr_err("%s : lip current_hbm_level = %d\n", __func__, enable);
	return;
}
...
{
 	struct proc_dir_entry *res;
	g_ctrl_pdata->current_hbm_level = 0;
	res = proc_create("panel_hbm_switch", S_IWUGO | S_IRUGO, NULL,
			  &panel_hbm_proc_fops);
	if (!res)
	{
		printk(KERN_INFO "failed to create /proc/panel_hbm_switch\n");
		return -ENOMEM;
	}

	printk(KERN_INFO "created /proc/panel_hbm_switch\n");
	return 0;
}
/** ZTE_MODIFY end **/

irqreturn_t amoled_error_handler(int irq, void *data)
{
	g_amoled_error_flag = 1;
	return IRQ_HANDLED;
}
...
static int mdss_dsi_ctrl_probe(struct platform_device *pdev)
{
	...
	if (index == 0)
	{
		ctrl_pdata->acl_set_workqueue = create_singlethread_workqueue("acl_set_workqueue");
		INIT_DELAYED_WORK(&ctrl_pdata->acl_set_work, acl_set_work_func);
		ctrl_pdata->hbm_set_workqueue = create_singlethread_workqueue("hbm_set_workqueue");
		INIT_DELAYED_WORK(&ctrl_pdata->hbm_set_work, hbm_set_work_func);
	...
	rc = devm_request_irq(&pdev->dev,
			gpio_to_irq(ctrl_pdata->disp_err_gpio),
			amoled_error_handler, IRQF_TRIGGER_RISING,
			"DSI_ERROR_GPIO", ctrl_pdata);
		if (rc) {
			pr_err("DSI_ERROR_GPIO request_irq failed.\n");
			goto error_pan_node;
		}
		disable_irq(gpio_to_irq(ctrl_pdata->disp_err_gpio));
				...
				struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	ctrl_pdata->disp_err_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
		"qcom,platform-err-gpio", 0);

	if (!gpio_is_valid(ctrl_pdata->disp_err_gpio))
		pr_err("%s:%d, lcd err gpio not specified\n",
						__func__, __LINE__);
