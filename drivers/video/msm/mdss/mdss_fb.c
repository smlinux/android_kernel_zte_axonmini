...
extern int panel_system_backlight_level;
				      ...
				      enum led_brightness value)
{
	...
	if (!IS_CALIB_MODE_BL(mfd) && (!mfd->ext_bl_ctrl || !value ||
		...
		panel_system_backlight_level = bl_lvl;
...
extern int g_amoled_error_flag;
static int __mdss_fb_display_thread(void *data)
{
	...
	while (1) {
		...
		if(g_amoled_error_flag == 1)
		{
			mdss_fb_set_backlight(mfd, panel_system_backlight_level);
			printk("__mdss_fb_display_thread g_amoled_error_flag %d set backlight\n", g_amoled_error_flag);
