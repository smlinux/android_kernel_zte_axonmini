...
int mdss_dsi_panel_acl(struct mdss_panel_data *pdata, int level)
{
	...
	if(level == 0)
	{
		if (ctrl->acl_off_cmds.cmd_cnt)
			mdss_dsi_panel_cmds_send(ctrl, &ctrl->acl_off_cmds);
	}
	else if(level == 1)
	{
		if (ctrl->acl_on_cmds.cmd_cnt)
...
static char led_acl[2] = {0x55, 0x2};	/* DTYPE_DCS_WRITE1 */

static struct dsi_cmd_desc backlight_cmd[] = {
   {{DTYPE_DCS_WRITE1, 1, 0, 0, 1, sizeof(led_pwm1)},
	led_pwm1,},
	{{DTYPE_DCS_WRITE1, 1, 0, 0, 1, sizeof(led_acl)},
	led_acl,}	
};
#if 1
static char led_mode[2] = {0x53, 0x28};	/* DTYPE_DCS_WRITE1 */
static struct dsi_cmd_desc backlight_mode_cmd = {
	{DTYPE_DCS_WRITE1, 1, 0, 0, 1, sizeof(led_mode)},
	led_mode
};
#endif

int panel_system_backlight_level = 0;
extern int panel_id_from_lk;
static void mdss_dsi_panel_bklt_dcs(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	struct dcs_cmd_req cmdreq;
	struct mdss_panel_info *pinfo;
	static int old_level = 50,old_level_bak=50;
	static int hbm_setting = 0;
	pinfo = &(ctrl->panel_data.panel_info);
	if (pinfo->dcs_cmd_by_left) {
		if (ctrl->ndx != DSI_CTRL_LEFT)
			return;
	...
	ctrl->current_backlight_level = panel_system_backlight_level;
	pr_err("%s: hbm_setting %d ctrl->current_backlight_level=%d level=%d  \n", __func__, hbm_setting, ctrl->current_backlight_level, level);
...
//	if(old_level == 0)
	...
	if(old_level==0 && ctrl->current_acl_level==0)
	{
		/*queue_delayed_work(ctrl->acl_set_workqueue,
			&(ctrl->acl_set_work),
			msecs_to_jiffies(100));*/
		
		//mdss_dsi_panel_acl(&(ctrl->panel_data), 0);
		led_acl[1] = 0;
	}
	...
	if(old_level_bak ==0 && level !=0)
	{
		led_mode[1] = 0x28;

		memset(&cmdreq, 0, sizeof(cmdreq));
		cmdreq.cmds = &backlight_mode_cmd;
		cmdreq.cmds_cnt = 1;
		cmdreq.flags = CMD_REQ_COMMIT | CMD_CLK_CTRL;
		cmdreq.rlen = 0;
		cmdreq.cb = NULL;
...
int mdss_dsi_panel_power_enable(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
        ...
        /* for +-5V begin */
    	if (gpio_is_valid(ctrl_pdata->disp_vdd_enp_gpio)) {
    		rc = gpio_request(ctrl_pdata->disp_vdd_enp_gpio,
    						"disp_vdd_enp_enable");
    		if (rc) {
    			pr_err("%s: request disp_vdd_enp_gpio failed, rc=%d\n", __func__, rc);
    		} else {
                gpio_value = gpio_get_value((unsigned int)(ctrl_pdata->disp_vdd_enp_gpio));
                pr_debug("%s: disp_vdd_enp_gpio gpio_value=%d\n", __func__, gpio_value);            
                gpio_direction_output(ctrl_pdata->disp_vdd_enp_gpio, gpio_value);//config gpio to output
                pr_err("%s: enable disp_vdd_enp_gpio \n", __func__);
    	...
    	if (gpio_is_valid(ctrl_pdata->disp_vdd_enn_gpio)) {
    		rc = gpio_request(ctrl_pdata->disp_vdd_enn_gpio,
    						"disp_vdd_enn_enable");
    		if (rc) {
    			pr_err("%s: request disp_vdd_enn_gpio failed, rc=%d\n", __func__, rc);
    		} else {
                gpio_value = gpio_get_value((unsigned int)(ctrl_pdata->disp_vdd_enn_gpio));
                pr_debug("%s: disp_vdd_enn_gpio gpio_value=%d\n", __func__, gpio_value);            
                gpio_direction_output(ctrl_pdata->disp_vdd_enn_gpio, gpio_value);//config gpio to output
                pr_err("%s: enable disp_vdd_enn_gpio \n", __func__);
    	...
    	if (gpio_is_valid(ctrl_pdata->disp_vddi_1p8_gpio)) {
    		rc = gpio_request(ctrl_pdata->disp_vddi_1p8_gpio,
    						"disp_vddi_1p8_gpio");
    		if (rc) {
    			pr_err("%s: request disp_vddi_1p8_gpio failed, rc=%d\n", __func__, rc);
    		} else {
                gpio_direction_output(ctrl_pdata->disp_vddi_1p8_gpio, 1);//config gpio to output
		...
		if (gpio_is_valid(ctrl_pdata->disp_vci_3p3_gpio)) {
    		rc = gpio_request(ctrl_pdata->disp_vci_3p3_gpio,
    						"disp_vci_3p3_gpio");
    		if (rc) {
    			pr_err("%s: request disp_vci_3p3_gpio failed, rc=%d\n", __func__, rc);
    		} else {
                gpio_direction_output(ctrl_pdata->disp_vci_3p3_gpio, 1);//config gpio to output
...
{
	pr_err("%s: Read back value from panel is %x %x %x %x\n",
							__func__, ctrl_pdata->status_buf.data[0],  ctrl_pdata->status_buf.data[1] , ctrl_pdata->status_buf.data[2],ctrl_pdata->status_value);
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->acl_off_cmds,
		"qcom,mdss-dsi-acl-off-command", "qcom,mdss-dsi-acl-command-state");

	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->acl_on_cmds,
		"qcom,mdss-dsi-acl-on-command", "qcom,mdss-dsi-acl-command-state");

	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->hbm_off_cmds,
		"qcom,mdss-dsi-hbm-off-command", "qcom,mdss-dsi-acl-command-state");

	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->hbm_on_cmds,
		"qcom,mdss-dsi-hbm-on-command", "qcom,mdss-dsi-acl-command-state");
...
{
	seq_printf(m, "SAMSUNG-S6E3FA3-NA-SAMSUNG-1920*1080\n");
	return 0;
}
...
static int panel_info_proc_init(void)
{
	...
	res = proc_create("driver/lcd_id",  0, NULL,
			  &panel_info_proc_fops);
	if (!res)
	{
		printk(KERN_INFO "failed to create /proc/driver/lcd_id\n");
		return -ENOMEM;
	}

	printk(KERN_INFO "created /proc/driver/lcd_id\n");
	return 0;
}
