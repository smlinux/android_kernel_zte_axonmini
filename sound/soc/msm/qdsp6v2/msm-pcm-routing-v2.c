...
extern int dolby_switch_status;
enum {
	MADNONE,
	MADAUDIO,
	MADBEACON,
	...
	for (i = 0; i < MSM_BACKEND_DAI_MAX; i++) {
		if (!is_be_dai_extproc(i) &&
		   ...
		   (test_bit(fedai_id, &msm_bedais[i].fe_sessions))) {
			...
			/*modify by yangchao for hp_hifi(24bit) 20150703 begin*/
			  if ((SNDRV_PCM_STREAM_PLAYBACK == stream_type) && (dolby_switch_status == 0))
			  {
				bits_per_sample = 24;

				pr_err("%s:PLAYBACK change to 24bits(%d)\n",__func__, bits_per_sample);
			  }
			/*modify by yangchao for hp_hifi(24bit) 20150703 end*/
	...
	for (i = 0; i < MSM_BACKEND_DAI_MAX; i++) {
		if (!is_be_dai_extproc(i) &&
		   ...
		   (test_bit(fedai_id, &msm_bedais[i].fe_sessions))) {
			...
			if (topology == 0){
				topology = msm_routing_get_adm_topology(path_type, fedai_id);
			}
	...
	if (set) {
		...
		if (msm_bedais[reg].active && fdai->strm_id !=
			...
			if (msm_bedais[reg].port_id == VOICE_RECORD_RX ||
				...
				topology = NULL_COPP_TOPOLOGY;
			/*modify by yangchao for hp_hifi(24bit) 20150703 begin*/
			if((SESSION_TYPE_RX == session_type) && (dolby_switch_status == 0))
			{
				bits_per_sample = 24;
		...
		if (msm_bedais[reg].active && fdai->strm_id !=
			...
			pr_debug("%s: copp: %ld, reset idx bit fe:%d, type: %d, be:%d topology=0x%x\n",
					__func__, copp, val, session_type, reg,topology);
			clear_bit(idx,
				  &session_copp_map[val][session_type][reg]);
			if ((DOLBY_ADM_COPP_TOPOLOGY_ID == topology ||
				DS2_ADM_COPP_TOPOLOGY_ID == topology) &&
	...
	for_each_set_bit(i, &bedai->fe_sessions, MSM_FRONTEND_DAI_MM_SIZE) {
		...
		if (fdai->strm_id != INVALID_SESSION) {
			...
			pr_debug("%s: copp:%ld,idx bit fe:%d, type:%d,be:%d topology=0x%x\n",
					__func__, copp, i, session_type, be_id,topology);
			clear_bit(idx,
				  &session_copp_map[i][session_type][be_id]);
			if ((fdai->perf_mode == LEGACY_PCM_MODE) &&
...
static int msm_pcm_routing_prepare(struct snd_pcm_substream *substream)
{
	...
	for_each_set_bit(i, &bedai->fe_sessions, MSM_FRONTEND_DAI_MM_SIZE) {
		...
		if (fdai->strm_id != INVALID_SESSION) {
			...
			/*modify by yangchao for hp_hifi(24bit) 20150703 begin*/
			  if ((playback) && (dolby_switch_status == 0))
			  {
			...
			/*modify by yangchao for hp_hifi(24bit) 201507003 end*/
			topology = msm_routing_get_adm_topology(path_type, i);
