...
static int msm_compr_configure_dsp(struct snd_compr_stream *cstream)
{
	...
	if (prtd->codec_param.codec.format == SNDRV_PCM_FORMAT_S32_LE)
		bits_per_sample = 32;
/*modify for yangchao for hp_hifi 20150703 begin*/
       if((dolby_switch_status == 0) && (((FORMAT_LINEAR_PCM == prtd->codec) || (FORMAT_MP3 == prtd->codec)/* || (FORMAT_FLAC == prtd->codec) */)
           && (44100 == prtd->sample_rate)) )
       {
           bits_per_sample = 24;
       }
/*modify for yangchao for hp_hifi 20150703 end*/
	if (prtd->compr_passthr != LEGACY_PCM) {
...
#else
		rc = q6asm_stream_open_write_v2(prtd->audio_client,
				prtd->codec, bits_per_sample, // chenjun
				stream_id,
				prtd->gapless_state.use_dsp_gapless_mode);
#endif
