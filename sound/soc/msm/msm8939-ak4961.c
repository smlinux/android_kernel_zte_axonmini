...
#define DRV_NAME "msm8939-slimbus-ak4961"
...
#else
static struct ak4961_mbhc_config ak4961_mbhc_cfg = {
...
/*	
	int (*mbhc_hs_detect) (struct snd_soc_codec *codec,
			       struct ak4961_mbhc_cfg *mbhc_cfg);
*/
};
					...
					bool dapm)
{
	...
	pr_info("[lhs]%s: enable = %d clk_users = %d\n",
	...
	if (enable) {
		...
		if (ret < 0) {
			pr_info("%s: failed to enable the MCLK\n",
							__func__);
			clk_users--;
			goto exit;
		}
...
#else
		if (!strcmp(card->name, "msm8939-ak4961-snd-card"))
...
enum hrtimer_restart oneline_timer_func(struct hrtimer *timer)
{
	...
	if(loop_times == 0){
		...
		loop_times--;
	    gpio_direction_output(ext_spk_amp_gpio, spk_enable);
	    spk_enable = !spk_enable;		
		//mutex_unlock(&oneline_mlock);
...
static int analog_pa_enable_set(struct snd_kcontrol *kcontrol,
                                       struct snd_ctl_elem_value *ucontrol)
{
    if(ucontrol->value.integer.value[0]) {
		//usleep_range(10000,10000);
		spk_enable = 1;
		loop_times = 2*ucontrol->value.integer.value[0]-1;
		hrtimer_start(&timer, ktime_set(0, 2000), HRTIMER_MODE_REL);

	}else {
	   gpio_direction_output(ext_spk_amp_gpio, 0);
	}
	spkpa_mode = ucontrol->value.integer.value[0];
    return 0;    
}

static int analog_pa_enable_get(struct snd_kcontrol *kcontrol,
                                       struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.enumerated.item[0] = spkpa_mode;
...
static char const *slim0_rx_sample_rate_text[] = {"KHZ_48", "KHZ_96",
					"KHZ_192"};
static char const *analog_pa_enable[] = {"0", "1", "2", "3", "4", "5", "6", "7"};
...
static const struct soc_enum msm_snd_enum[] = {
	...
	SOC_ENUM_SINGLE_EXT(8, analog_pa_enable),
};
...
static const struct snd_kcontrol_new msm_snd_controls[] = {
	...
	SOC_ENUM_EXT("Analog PA Enable", msm_snd_enum[5],
			analog_pa_enable_get, analog_pa_enable_set),
...
{
	pr_err("[YCH]%s: ADSP state %d.\n", __func__,(int)value);
	if (value == SUBSYS_BEFORE_SHUTDOWN) {
		pr_debug("[YCH]%s: ADSP is about to shutdown. Clearing AFE config\n",__func__);
		msm_afe_clear_config();
	} else if (value == SUBSYS_AFTER_POWERUP) {
		pr_debug("[YCH]%s: ADSP is up\n", __func__);
	}else if(value == SUBSYS_AFTER_SHUTDOWN){
		pr_debug("[YCH]%s: ADSP is after shutdown.\n", __func__);
	}
...
static int msm8939_ak4961_codec_up(struct snd_soc_codec *codec)
{
	int err;
	bool timedout;
	...
	if (!q6core_is_adsp_ready()) {
		...
		dev_err(codec->dev,
			"%s: DSP is ready\n", __func__);
		//pr_err("[YCH]%s: DSP is ready\n", __func__);
	}

	if (!adsp_ready) {
		pr_err("[YCH]%s: timed out waiting for ADSP Audio\n", __func__);
		return -ETIMEDOUT;
	}
		...
		enum ak49xx_codec_event codec_event)
{
	...
	case AK49XX_CODEC_EVENT_CODEC_UP:
		...
		return msm8939_ak4961_codec_up(codec);
	default:
...
static int msm_audrx_init_tomtom(struct snd_soc_pcm_runtime *rtd)
{
	...
	if (wcd9xxx_mbhc_cfg.calibration) {
		...
		pr_err("%s: ak4961_mbhc_cfg calibration is NULL\n", __func__);
		err = -ENOMEM;
		goto out;
	}
#endif
...
static int msm_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	snd_soc_dapm_ignore_suspend(dapm, "SPK_OUT");
/*ZTE_modify add for no sound after call suspend 0429 begin*/
...
/*ZTE_modify add for no sound after call suspend 0429 end*/
	snd_soc_dapm_sync(dapm);
	...
	ret = ak4961_hs_detect(codec, &ak4961_mbhc_cfg);
	if (ret) {
...
#else
	ak4961_event_register(msm8939_codec_event_cb, rtd->codec);
#endif
...
static struct snd_soc_dai_link msm8x16_4961_dai[] = {
	/* Backend DAI Links */
	{
...
static struct snd_soc_dai_link msm8x16_4961_dai_links[
				ARRAY_SIZE(msm8x16_dai) +
				ARRAY_SIZE(msm8x16_4961_dai)];
/*
			...
			struct msm8939_asoc_mach_data *pdata)
{
	...
	if (!(pdata->ext_pa & QUAT_MI2S_ID)) {
		printk("[lhs] %s enable device tree\n",__func__);
...
cpu_dai:
		...
		if (dai_link[i].cpu_dai_name && !dai_link[i].cpu_of_node) {
			...
			phandle = of_parse_phandle(cdev->of_node, "asoc-cpu",
					      index);
			if (!phandle) {
				pr_info("[lhs]%s: retrieving phandle for cpu dai %s failed\n",
...
codec_dai:
		...
		if (dai_link[i].codec_name && !dai_link[i].codec_of_node) {
			...
			phandle = of_parse_phandle(cdev->of_node, "asoc-codec",
					      index);
			if (!phandle) {
				pr_info("[lhs]%s: retrieving phandle for codec dai %s failed\n",
...
static int msm8939_asoc_machine_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "[lhs]card name = %s\n",card->name);
	if (!strcmp(card->name, "msm8939-ak4961-snd-card")) {
		snd_soc_card_msm[AK4961_CODEC].name = card->name;
		card = &snd_soc_card_msm[AK4961_CODEC];
		memcpy(msm8x16_4961_dai_links, msm8x16_dai,
				sizeof(msm8x16_dai));
		memcpy(msm8x16_4961_dai_links + ARRAY_SIZE(msm8x16_dai),
			msm8x16_4961_dai, sizeof(msm8x16_4961_dai));
		card->dai_link	= msm8x16_4961_dai_links;
		card->num_links	= ARRAY_SIZE(msm8x16_4961_dai_links);
		//pdata->msm8939_codec_fn.mclk_enable_fn = ak4961_mclk_enable;
		//pdata->msm8939_codec_fn.get_afe_config_fn = ak4961_get_afe_config;
		//pdata->msm8939_codec_fn.mbhc_hs_detect = ak4961_hs_detect;
	...
	ak4961_mbhc_cfg.gpio_level_insert = of_property_read_bool(
						pdev->dev.of_node,
	...
	if (pdata->us_euro_gpio < 0) {
		...
		ak4961_mbhc_cfg.swap_gnd_mic = msm8939_swap_gnd_mic;
	}
	...
	ext_spk_amp_gpio = of_get_named_gpio(pdev->dev.of_node,
			"qcom,ext-spkr-pa-gpios-pin", 0);
	if (ext_spk_amp_gpio < 0) {
		dev_err(&pdev->dev,"Get %s failed\n","qcom,ext-spkr-pa-gpios-pin");
	} else {
		ret = gpio_request(ext_spk_amp_gpio,"EXT_SPK_PA_GPIO");
		if (ret) {
