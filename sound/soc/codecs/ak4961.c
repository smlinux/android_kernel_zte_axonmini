...
#define AK4961_NUM_PRAM			6
#define AK4961_NUM_CRAM			18  //barge-in has no cram
#define AK4961_NUM_ORAM			3
#ifdef CONFIG_SND_SOC_ES705_ESCORE
...
static const char *AK4961_CRAM_FIRMWARES[] = {
	...
	"ak4961_cram_narrow_headset.bin",
	...
	"ak4961_cram_barge_in.bin",
	"ak4961_cram_karaoke_cave.bin",
	"ak4961_cram_karaoke_cloister.bin",
	"ak4961_cram_karaoke_live.bin",
	"ak4961_cram_karaoke_room.bin",
	"ak4961_cram_karaoke_theater.bin",
	"ak4961_cram_karaoke_valley.bin",
	"ak4961_cram_karaoke_bypass.bin"
};
...
#define AK4961_MCLK_RATE_19200KHZ 19200000

#define AK4961_RX_PORT_START_NUMBER	10

static int last_crc;

static struct snd_soc_codec *ak4961_codec;
...
//static int slim_ssr_es = 0;
static int ak_ctrl_pdata = 0;
static int ak4961_norrow_wide_bandswitch(int value);
/*
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	if(debugak49xx==NULL)
		{
			pr_err("%s: read register pr = NULL error.\n", __func__);
			return sprintf(buf, "pr =NULL error"); 
		}
...

{
	...
	pr_debug("%s(): dai_name = %s DAI-ID %x tx_ch %d rx_ch %d\n"
		 ...
		 ak4961->intf_type);
	/*pr_err("[YCH]%s(): dai_name = %s DAI-ID %x tx_ch %d rx_ch %d\n"
		 "ak4961->intf_type %d\n",
		 __func__, dai->name, dai->id, tx_num, rx_num,
		 ak4961->intf_type);*/
...
/*ZTE_modify add es705 dapm by yangchao 0313 begin*/
...
static int ak4961_esxxx_startup(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	ak4961_startup(substream, dai);
	return 0;
}

static void ak4961_esxxx_shutdown(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	ak4961_shutdown(substream, dai);
}

static int ak4961_esxxx_hw_params(struct snd_pcm_substream *substream,
		struct snd_pcm_hw_params *params,
		struct snd_soc_dai *dai)
{
	int rc = 0;
	...
	rc = ak4961_hw_params(substream, params, dai);
...
/* ZTE_modify add es704 driver begin */
static int ak4961_esxxx_set_channel_map(struct snd_soc_dai *dai,
				unsigned int tx_num, 
				...
				unsigned int *rx_slot)
{
	...
	if (remote_route_enable(dai)) {
		rc = ak4961_get_channel_map(dai, &taiko_tx_num, taiko_tx_slot,
					&taiko_rx_num, taiko_rx_slot);
		if (rc < 0)
			pr_err("%s: taiko_get_channel_map error %d\n",
			       __func__, rc);

		rc = ak4961_set_channel_map(dai, tx_num,
...
static int ak4961_esxxx_get_channel_map(struct snd_soc_dai *dai,
				       unsigned int *tx_num,
				       unsigned int *tx_slot,
				       unsigned int *rx_num,
				       unsigned int *rx_slot)

{
	...
	if (remote_route_enable(dai))
		...
		rc = ak4961_get_channel_map(dai, tx_num, tx_slot,
					rx_num, rx_slot);
...
static struct snd_soc_dai_ops ak4961_dai_ops = {
	.startup = ak4961_esxxx_startup, /* taiko_startup, */
	.shutdown = ak4961_esxxx_shutdown, /* taiko_shutdown, */
	.hw_params = ak4961_esxxx_hw_params, /* taiko_hw_params, */
	.set_sysclk = ak4961_set_sysclk,
	.set_fmt = ak4961_set_fmt,
	.set_pll = ak4961_set_fll,
	.digital_mute	= ak4961_digital_mute,
	.set_channel_map = ak4961_esxxx_set_channel_map,
	.get_channel_map = ak4961_esxxx_get_channel_map,
	.trigger = ak4961_trigger,
};
#else
/*ZTE_modify add es705 dapm by yangchao 0313 end*/
static struct snd_soc_dai_ops ak4961_dai_ops = {
...
/* ZTE_MODIFY for smartpa mode sel by lvrg begin */
static const char *smartpa_mode[] = {
       "0", "1", "2"
};

static const struct soc_enum smarpa_mode_sel =
       SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(smartpa_mode), smartpa_mode);
/* ZTE_MODIFY for smartpa mode by lvrg end */
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ak4961_dsp_mode >= DSP_MODE_SOUND_RECORD &&
		ak4961_dsp_mode <= DSP_MODE_SOUND_RECORD) {   //add condition
			val[0] = 0x81;
			val[1] = RUN_STATE_DATA_LENGTH >> 8;
			val[2] = RUN_STATE_DATA_LENGTH & 0xff;
			val[3] = 0x00;
...
static const char *dsp_mode_text[] = {
	...
	"wide_handset", "wide_headset", "wide_handsfree", "wide_headphone",
	"voice_recognition",
...
static const u16 ram_table[] = {
	...
	0x140, 0x150, 0x160, 0x170,	// WIDE_MODE
	0x280,					// Voice Recognition
	0x390, 0x3A0,			// Sound Voice Record
	0x4B0,					// Music Speaker
	0x500,					// Barge-in
	0x6C1, 0x6D2, 0x6E3		// Karaoke
};
#endif
static const u16 ram_table[] = {
	...
	0x3090,
	0x40a0,
	0x50b0, 0x50c1,	0x50d2,		// karaoke
	0x50e0, 0x50f1,	0x5102, 0x5112		// karaoke
};
        ...
        struct snd_ctl_elem_value *ucontrol)
{
	...
	if (new_dsp_mode != DSP_MODE_OFF) {
		...
		pr_err("[lhs]%s: pram_index = %d, cram_index =%d, oram_index = %d\n",
		...
		printk("[lhs] %s cram_index=%d ex_cram_index=%d \n",__func__,cram_index,ex_cram_index);
		if(ak4961->cram_firmware[cram_index] == NULL)
			{
			printk("[lhs]%s cram %d  = null \n",__func__,cram_index);
			}
	...
	if (ret == 0) {
		...
		case DSP_MODE_NARROW_HEADPHONE:
			...
			//err = ak4961_norrow_wide_bandswitch(ak_ctrl_pdata);
			break;
...
#if 1   //del for voice recognize and record same dsp setting ,but different firmware by lvrongguo
			snd_soc_write(codec, DSP_SETTING1, 0x68);
			snd_soc_write(codec, DSP_SETTING2, 0x3C);
...
//			snd_soc_write(codec, FLOW_CONTROL_3, 0x01);
	...
	if (ak4961_dsp_mode >= DSP_MODE_SOUND_RECORD &&
		ak4961_dsp_mode <= DSP_MODE_SOUND_RECORD) {
		val[0] = 0x81;
		val[1] = RUN_STATE_DATA_LENGTH >> 8;
		val[2] = RUN_STATE_DATA_LENGTH & 0xff;
		val[3] = 0x00;
        ...
        struct snd_ctl_elem_value *ucontrol) 
{
     ...
     printk("YCH: %s enter wide bandswitch %d\n", __func__, ak_ctrl_pdata);
     ak4961_norrow_wide_bandswitch(ak_ctrl_pdata);
     return 0;
}
...
static const char *lch_bargein_sel_text[] = {
	"n/a", "output1_l", "output1_r", "output3_l", "output3_r",
	"input5_l", "input5_r", "input5_m"
};

static const char *rch_bargein_sel_text[] = {
	"n/a", "output1_l", "output1_r", "output3_l", "output3_r",
	"input5_l", "input5_r", "input5_m"
};

static const struct soc_enum lch_bargein_sel_text_enum =
		SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(lch_bargein_sel_text), lch_bargein_sel_text);

static const struct soc_enum rch_bargein_sel_text_enum =
		SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(rch_bargein_sel_text), rch_bargein_sel_text);
...
/* ZTE_MODIFY for smartpa mode sel by lvrg begin */
static int smartpa_set_model_sel(struct snd_kcontrol *kcontrol,
                                       struct snd_ctl_elem_value *ucontrol)
{
       struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
       struct ak4961_priv *ak4961 = snd_soc_codec_get_drvdata(codec);
    ...
    ak4961->smartpa_mode_flag = status;

       return 0;
       
}

static int smartpa_get_model_sel(struct snd_kcontrol *kcontrol,
                                       struct snd_ctl_elem_value *ucontrol)
{
       struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
       struct ak4961_priv *ak4961 = snd_soc_codec_get_drvdata(codec);

       ucontrol->value.enumerated.item[0] = ak4961->smartpa_mode_flag;

       return 0;

}
/* ZTE_MODIFY for smartpa mode sel by lvrg end */
...
static const struct snd_kcontrol_new ak4961_snd_controls[] = {
    ...
    SOC_ENUM_EXT("Tfa9890 Mode Sel", smarpa_mode_sel,
                       smartpa_get_model_sel, smartpa_set_model_sel),	
		...
		struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	int reg_srcab,reg_srccd;
/* modify for resolved second voice call coming while voice call no sound  begin */
	if(ak4961_dsp_mode != DSP_MODE_OFF)
		return 0;
/* modify for resolved second voice call coming while voice call no sound  end */
...
static irqreturn_t ak4961_jde_irq(int irq, void *data)
{
	...
	if (val < 0) {
		...
		dev_err(codec->dev, "Failed to read JACK_DETECTION_STATUS: %d\n",
			val);
		pr_err("[YCH]Failed to read JACK_DETECTION_STATUS: %d\n",
			val);
		return IRQ_HANDLED;
	}
	printk("[YCH] %s val%d\n",__func__,val);
		    ...
		    const struct ak4961_mbhc_config *cfg)
{
	...
	case AK4961_MCLK_RATE_9600KHZ:
		...
		pr_err("[YCH]MCLK: clock rate using %dHz\n", cfg->mclk_rate);
		break;
	default:
...
static const struct ak4961_reg_mask_val ak4961_codec_reg_init_val[] = {
	...
	//{MSYNC2_MSN_CKS, 0x3F, 0x21},   //modify for smartPA
...
//	{BUSMCLK_DIVIDER, 0xFF, 0x03},
	...
	/*mclk freq divided cfg added by yangchao 20150718*/
...
static int ak4961_device_down(struct ak49xx *ak49xx)
{
	...
	//printk("YCH, %s : finish online set\n", __func__);
	return 0;
}
...
static int ak4961_post_reset_cb(struct ak49xx *ak49xx)
{
	...
	//ak4961_hs_detect(codec,&ak4961->mbhc_cfg);// lhs add  for head jack detect
	
	ak4961->machine_codec_event_cb(codec, AK49XX_CODEC_EVENT_CODEC_UP);

	mutex_unlock(&codec->mutex);

	//printk("YCH: %s finish", __func__);
	return ret;
}
...
#if 0
int g_audio_boot_ffbm = 0;
static int __init ffbm_set(char *str)
{
    ...
    if(strstr(str,"ffbm-01")) 
    {
	    ...
	    printk("[jjp] %s find ffbm-01\n",__func__);
        g_audio_boot_ffbm = 1;
    }
	return 1;
}
__setup("androidboot.mode=", ffbm_set);
#endif
static void ak4961_oram_ready(const struct firmware *fw, void *context)
{
	...
	if (!fw) {
		...
		dev_err(codec->dev, "ORAM firmware %d request succeed\n",ak4961->oram_load_index);
	}
	ak4961->oram_load_index++;
...
static void ak4961_pram_ready(const struct firmware *fw, void *context)
{
	...
	if (!fw) {
		...
		dev_err(codec->dev, "PRAM firmware %d request succeed\n",ak4961->pram_load_index);
	}
	ak4961->pram_load_index++;
...
static void ak4961_cram_ready(const struct firmware *fw, void *context)
{
	...
	if (!fw) {
		...
		dev_err(codec->dev, "[lhs]CRAM firmware %d request succeed\n",ak4961->cram_load_index);
	}
	ak4961->cram_load_index++;
...
/*ZTE_modify add es705 dapm by yangchao 0313 end*/
    ...
    if(!g_boot_ffbm) // modefy by jjp for reboot in pv
    {
    	request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
...
/*add by shengguanghui for voice_wn switch 20150604 begin*/

int ak4961_norrow_wide_bandswitch(int howtochange)
{
	struct snd_soc_codec *codec = ak4961_codec;
	int mir3_1,mir3_2,mir3_3,mir3_4;
         //int howtochange = 0;//ucontrol->value.integer.value[0];
	//int mir3=0;
	printk("YCH: enter %s howtochange = %d\n", __func__, howtochange);
	...
	//mir3=mir3_1+mir3_2+mir3_3+mir3_4;

	pr_err("[lhs] %s: howtochange(%d), mir3_1(%x), mir3_2(%x), mir3_3(%x), mir3_4(%x),\n",
                        __func__, howtochange, mir3_1, mir3_2, mir3_3, mir3_4);

        switch(mir3_2)
        {
            case 0:
                pr_err("[lhs] %s set to handset mode\n",__func__);
                // in handset mode need change the PEQ
                //if N to W
                if(howtochange==2){//if N to W
                ...
                {//else W to N
                    ...
                    snd_soc_write(codec,RUN_STATE_START_ADDR1,0x05);
                    snd_soc_write(codec,RUN_STATE_START_ADDR2,0x96);
                    snd_soc_write(codec,RUN_STATE_DATA_1,0x3d);
                    snd_soc_write(codec,RUN_STATE_DATA_2,0x0c);
                    snd_soc_write(codec,RUN_STATE_DATA_3,0x90);
                    snd_soc_write(codec,RUN_STATE_DATA_4,0x85);
                    snd_soc_write(codec,RUN_STATE_DATA_5,0xe6);
                    snd_soc_write(codec,RUN_STATE_DATA_6,0xf0);
                    snd_soc_write(codec,RUN_STATE_DATA_7,0xc5);
                    snd_soc_write(codec,RUN_STATE_DATA_8,0xc4);
                    snd_soc_write(codec,RUN_STATE_DATA_9,0x10);
                    snd_soc_write(codec,RUN_STATE_DATA_10,0x79);
                    snd_soc_write(codec,RUN_STATE_DATA_11,0xf6);
                    snd_soc_write(codec,RUN_STATE_DATA_12,0x40);
                    snd_soc_write(codec,RUN_STATE_DATA_13,0x3d);
                    snd_soc_write(codec,RUN_STATE_DATA_14,0x0c);
                    snd_soc_write(codec,RUN_STATE_DATA_15,0x90);
                    snd_soc_write(codec,CRAM_RUN_EXE,0x01);
                }
                break;

            case 1:
                pr_err("[lhs]%s handfree mode \n",__func__);
                if(howtochange==2)//if N to W
                {
                    snd_soc_write(codec,RUN_STATE_DATA_LENGTH,0x00);
                    snd_soc_write(codec,RUN_STATE_START_ADDR1,0x06);
                    snd_soc_write(codec,RUN_STATE_START_ADDR2,0x18);
                    snd_soc_write(codec,RUN_STATE_DATA_1,0x01);
                    snd_soc_write(codec,RUN_STATE_DATA_2,0xAC);
                    snd_soc_write(codec,RUN_STATE_DATA_3,0x00);
                    snd_soc_write(codec,CRAM_RUN_EXE,0x01);
                }
                else if(howtochange==1)//else W to N
                {
                    snd_soc_write(codec,RUN_STATE_DATA_LENGTH,0x00);
                    snd_soc_write(codec,RUN_STATE_START_ADDR1,0x06);
                    snd_soc_write(codec,RUN_STATE_START_ADDR2,0x18);
                    snd_soc_write(codec,RUN_STATE_DATA_1,0x01);
                    snd_soc_write(codec,RUN_STATE_DATA_2,0xCC);
                    snd_soc_write(codec,RUN_STATE_DATA_3,0x00);
                    snd_soc_write(codec,CRAM_RUN_EXE,0x01);
                }
                break;
            ...
            case 2:
                ...
                if(howtochange==2)//if N to W
                {
                    ...
                    snd_soc_write(codec,RUN_STATE_START_ADDR2,0x18);
                    snd_soc_write(codec,RUN_STATE_DATA_1,0x01);
                    snd_soc_write(codec,RUN_STATE_DATA_2,0xDC);
                    snd_soc_write(codec,RUN_STATE_DATA_3,0x00);
                    snd_soc_write(codec,CRAM_RUN_EXE,0x01);
                }
                else if(howtochange==1)//else W to N
                {
                    ...
                    snd_soc_write(codec,RUN_STATE_START_ADDR2,0x18);
                    snd_soc_write(codec,RUN_STATE_DATA_1,0x01);
                    snd_soc_write(codec,RUN_STATE_DATA_2,0xFC);
                    snd_soc_write(codec,RUN_STATE_DATA_3,0x00);
                    snd_soc_write(codec,CRAM_RUN_EXE,0x01);
                }
                break;
...
static int voice_wn_proc_init(void)
{
	...
	if (!res)
	{
		printk(KERN_INFO "failed to create /proc/voice_wn_switch\n");
		return -ENOMEM;
	}

	printk(KERN_INFO "created /proc/voice_wn_switch\n");
	return 0;
}
/*add by shengguanghui for voice_wn switch 20150604 end*/

int dolby_switch_status = 0;

static ssize_t dolby_proc_read(struct file *file, char __user *buf,
				    size_t len, loff_t *offset)
{
    ssize_t count = 0;
	...
	if (*offset > 0) {
		...
		pr_err("%s: enter %d\n", __func__, dolby_switch_status);
		count = sprintf(buf, "%d\n", dolby_switch_status);
		*offset += count;
    }
    return count;
}
static ssize_t dolby_proc_write(struct file *file, const char __user *buffer,
				    size_t count, loff_t *pos)
{
	int ret;
	int rc;
	...
	dolby_switch_status = ret;

	pr_err("%s: dolby_switch_status = %d\n", __func__, dolby_switch_status);
	return 1;

}


static const struct file_operations dolby_proc_fops = {
	.owner  = THIS_MODULE,
	.read		= dolby_proc_read,
	.write		= dolby_proc_write,
};

static int dolby_proc_init(void)
{
	struct proc_dir_entry *res;
	res = proc_create("dolby_switch",S_IFREG|S_IRWXUGO,NULL,
			  &dolby_proc_fops);
	if (!res)
	{
		printk(KERN_INFO "failed to create /proc/dolby_switch\n");
		return -ENOMEM;
	}

	printk(KERN_INFO "created /proc/dolby_switch\n");
	return 0;
}
...
static int ak4961_probe(struct platform_device *pdev)
{
	...
	dolby_switch = dolby_proc_init();
        /* zte jjp add headset detect /proc/hs 2015-03-30 start */
        proc_hs_type = proc_create("hs", S_IFREG | S_IRUGO, NULL,
        &hs_proc_fops);
        if (!proc_hs_type) 
        {
